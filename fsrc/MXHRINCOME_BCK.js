...
// Required state for template parsing and export
let columnMap = [];
...
// Keep only one colLetter (remove if defined inside exportReport)
const colLetter = (n) => {
    let s = "", q = n;
    while (q > 0) {
        const r = (q - 1) % 26;
...
/* =============================== Export =============================== */
// async function exportReport(data, filter, templateFile, resultFile) {
//     if (!data || !data.length) {
...
//         message += `Row ${index + 1}:\nREFNO: ${row.REFNO}\nPayToName: ${row.PayToName}\n\n`;
//     });
//     //alert(message);
//     //return;
//     //
//     //setStatus("Exporting...");
//     const tplRes = await fetch( templateFile, { cache: "no-store" }); //"template.xlsx"
...
//     // 1) Period line
//     const filterText = describeFilter(filter, "For Period ", "dd/mm/yyyy", false);
//     ws.getRow(periodRow).getCell(1).value = filterText;
//     ws.getRow(periodRow).commit();

//     // const filterText = describeFilter(filter); // ✅ แปลง filter เป็นข้อความ
//     // ws.getRow(periodRow).getCell(1).value = `Filtered by: ${filterText}`;
//     // ws.getRow(periodRow).commit();
...
//             const colIdx = colMap[key.toLowerCase()];
...
//     saveAs(new Blob([out]), resultFile); //"report.xlsx"
//     //setStatus("Export complete");
// }

/* async function exportReportNN(data, filter) {
    if (!data || !data.length) {
    ...
    parseTemplateWorkbook(wb); // เตรียม columnMap, placeholderRowIdx ฯลฯ
    ...
    // 🟡 เขียน filter ที่ผู้ใช้เลือกไว้
    ...
    columnMap.forEach(cm => {
      if (!cm.placeholder) return;
      const cell = hdrRow.getCell(cm.colIndex);
      cell.value = templateColHeaders[cm.colIndex] || "";
    });
    hdrRow.commit();
  
    // 🟢 สร้าง map placeholder → column index
    const colMap = {};
    columnMap.forEach(c => {
      if (c.placeholder) colMap[c.placeholder] = c.colIndex;
    });
    ...
    data.forEach((rowData, i) => {
      ...
      for (const [key, val] of Object.entries(rowData)) {
        ...
        const dtType = columnMap.find(c => c.placeholder === key.toLowerCase())?.dataType || guessDataType(key);
        const cell = row.getCell(colIdx);
    ...
    // 🟢 เขียน totals row (เฉพาะ directive)
    const totalsRow = ws.getRow(totalsRowIndex);
    columnMap.forEach(cm => {
      if (!cm.placeholder || !cm.directive) return;
  
      const colIdx = cm.colIndex;
      ...
      if (formula) totalsRow.getCell(colIdx).value = { formula };
    });
    totalsRow.commit();
...
//     const tplRes = await fetch(templateFile, { cache: "no-store" });
//     const buf = await tplRes.arrayBuffer();
//     const wb = new ExcelJS.Workbook();
//     await wb.xlsx.load(buf);
//     parseTemplateWorkbook(wb); // assumes columnMap, placeholderRowIdx, etc. are set
...
//     // ✅ Load DeptCode → DeptName mapping from MSTABLE
//     const deptMap = {};
//     const msTable = wb.getWorksheet("MSTABLE");
//     if (msTable) {
//         msTable.eachRow((row, rowIndex) => {
...
//     //         // ✅ Replace DeptCode with DeptName
//     //         if (key.toLowerCase() === "department") {
//     //             const deptName = deptMap[String(val).trim()] || val;
//     //             cell.value = deptName;
//     //         } else if (dtType === "date") {
//     //             const d = val instanceof Date ? val : new Date(val);
//     //             cell.value = Number.isNaN(d.getTime()) ? "" : d;
...
// // กำหนด templateRow = แถวแรกของข้อมูลจริง
...
//             // ✅ copy style จาก firstDataRow (templateRow)
...
//     // ✅ วางตรงนี้ เพื่อหาขอบเขตคอลัมน์ของข้อมูลจริง
//     const colIndexes = columnMap
...
//     // alert(`colStart ${colStart}, colEnd ${colEnd}`)
//     // // ✅ ใช้กับ border หรือ format ได้เลย เช่น
//     // applyDashedBorderRange(ws, firstDataRow, lastDataRow, colStart, colEnd);
...
// }
// 🟢 Helper: แปลงค่า cell.value ให้อ่านง่าย
...
async function exportReport(data, filter, templateFile, resultFile) {
    ...
    // ---------------------------
    // ✅ Add: DevExtreme filter evaluator (non-destructive)
    // ---------------------------
    ...
    const getFieldValue = (row, field) => {
        ...
        // direct match or case-insensitive field name
    ...
    const compare = (left, op, right) => {
        // Try to normalize numbers/dates but do not force when not applicable
        const lNum = toNumberIfPossible(left), rNum = toNumberIfPossible(right);
        const lDate = toDateIfPossible(left), rDate = toDateIfPossible(right);

        const bothNum = typeof lNum === 'number' && typeof rNum === 'number';
        const bothDate = (lDate instanceof Date) && (rDate instanceof Date);

        const L = bothNum ? lNum : (bothDate ? lDate.getTime() : left);
        const R = bothNum ? rNum : (bothDate ? rDate.getTime() : right);
        ...
        switch (op) {
            ...
            case 'between':
                // DevExtreme: ['field','between',[start,end]]
                if (Array.isArray(R) && R.length === 2) {
                    return compare(L, '>=', R[0]) && compare(L, '<=', R[1]);
                }
                return false;
    ...
    const evalDxFilter = (row, f) => {
        ...
        // logical groups: ['and', cond1, cond2, ...] or ['or', ...]
        if (typeof f[0] === 'string' && (f[0].toLowerCase() === 'and' || f[0].toLowerCase() === 'or')) {
            const op = f[0].toLowerCase();
            const parts = f.slice(1);
            if (op === 'and') return parts.every(p => evalDxFilter(row, p));
            else return parts.some(p => evalDxFilter(row, p));
        }

        // simple condition: ['field', 'op', value] or nested arrays combined implicitly with 'and'
        if (Array.isArray(f[0])) {
            // implicit AND between conditions: [cond1, 'and', cond2, 'and', cond3] or [[...],[...]]
            // Normalize: evaluate sequentially respecting 'and'/'or' tokens if present
    ...
    const applyDxFilter = (rows, dxFilter) => {
        if (!dxFilter) return rows; // ✅ No new filter → do nothing
        return rows.filter(r => evalDxFilter(r, dxFilter));
    };

    // ✅ Decide the exact dataset to export
    const expData = filter ? applyDxFilter(data, filter) : data;
    if (filter && (!expData || !expData.length)) {
    ...
    const headerRow = placeholderRowIdx - 1;
    const firstDataRow = placeholderRowIdx;
    const lastDataRow = firstDataRow + expData.length - 1; // ✅ use expData
    const totalsRowIndex = lastDataRow + 1;
    const periodRow = headerRow - 1;
    ...
    // กำหนด templateRow = แถวแรกของข้อมูลจริง
    const templateRow = ws.getRow(firstDataRow);
    expData.forEach((rowData, i) => { // ✅ use expData
        const rNo = firstDataRow + i;
        const row = ws.getRow(rNo);
        ...
        for (const [key, val] of Object.entries(rowData)) {
            ...
            // ✅ copy style จาก firstDataRow (templateRow)
            //const templateCell = templateRow.getCell(colIdx);
            //copyCellStyle(templateCell, cell); //old      
        }
        // ✅ Copy style จาก templateRow → row ครบทุก cell
        applyRowStyleFromTemplate(templateRow, row, columnMap);
    ...
    //     const hCell = headerTemplateRow.getCell(cm.colIndex);
    //     copyCellStyle(hCell, tCell); //old
    // });

    
    
    // 🟢 เขียน totals row (inline version)
    const totalsRow = ws.getRow(totalsRowIndex);
    const headerTemplateRow = ws.getRow(headerRow);
    ...
    columnMap.forEach(cm => {
        ...
        const tCell = totalsRow.getCell(cm.colIndex);
        const mergeInfo = ws._merges && ws._merges[tCell.address];
        const masterCell = mergeInfo ? ws.getCell(mergeInfo.top + 1, mergeInfo.left + 1) : tCell;

        const origValue = masterCell.value;
        const tmplInfo = valueInfo(masterCell);
        const tmplText = (tmplInfo.text || "").trim();
        const tmplIsDirective = isDirectiveToken(tmplText);

        if (tmplText && !tmplIsDirective) {
            // preserve free text
            masterCell.value = origValue;
            console.log(`Preserve free text at ${masterCell.address}: "${tmplText}"`);
            ...
            else if (cm.directive === "text") {
                if (cm.label) {
                    masterCell.value = cm.label;
                    console.log(`Set text at ${masterCell.address}: "${cm.label}"`);
                }
            }

            const needsFormula = (!tmplText || tmplIsDirective);
            if (needsFormula && formula) {
                masterCell.value = { formula };
                console.log(`Set formula at ${masterCell.address}: ${formula}`);
            }
        }

        // copy style
        const hCell = headerTemplateRow.getCell(cm.colIndex);
        copyCellStyle(hCell, masterCell);
    });
...
/* async function exportReportOOO(data, filter, templateFile, resultFile) {
    if (!data || !data.length) {
        alert("No data to export");
        return;
    }
    ...
    // ✅ Map placeholder → colIndex
    const colMap = {};
    columnMap.forEach(c => {
        if (c.placeholder) colMap[c.placeholder] = c.colIndex;
    });

    // ✅ Template row = firstDataRow
    const templateRow = ws.getRow(firstDataRow);
...
//     // ตัวอย่าง filter: ["Department", "=", "IT"]
//     if (Array.isArray(filter)) {
//         if (typeof filter[0] === "string") {
//             return `${filter[0]} ${filter[1]} ${filter[2]}`;
//         }

//         // ตัวอย่าง filter แบบซับซ้อน: [["Department", "=", "IT"], "and", ["Amount", ">", 1000]]
//         return filter.map(f => {
//             if (Array.isArray(f)) return `${f[0]} ${f[1]} ${f[2]}`;
...
// function describeFilterOO(filter) {
//     if (!filter) return "ไม่มีการกรอง";
...
// 🟢 ฟังก์ชัน parseTemplateWorkbook
function parseTemplateWorkbook(wb) {
    const ws = wb.getWorksheet("Report");
    if (!ws) throw new Error("Worksheet 'Report' not found in template.xlsx");
    ...
    for (let c = 1; c <= maxCols; c++) {
        const phRaw = phArr[c - 1], dirRaw = dirArr[c - 1];
        let placeholder = null, directive = null, label = null;
        ...
        if (typeof dirRaw === "string") {
            // รองรับ {sum}, {average}, {count}, {text}, {text: Some Label}
            const dm = dirRaw.match(/\{\s*(sum|average|count|text)(?::\s*([^}]+))?\s*\}/i);
            if (dm) {
                directive = dm[1].toLowerCase();
                if (directive === "text" && dm[2]) {
                    label = dm[2].trim();
                }
            }
        }
...
// function applyDashedBorderRange(ws, rowStart, rowEnd, colStart, colEnd) {
...
//             if (r === rowStart) border.top = { style: 'thin', color: { argb: 'FF999999' } };
//             if (r === rowEnd) border.bottom = { style: 'thin', color: { argb: 'FF999999' } };
//             if (c === colStart) border.left = { style: 'thin', color: { argb: 'FF999999' } };
//             if (c === colEnd) border.right = { style: 'thin', color: { argb: 'FF999999' } };
...
// function applyDashedBorderRange(ws, rowStart, rowEnd, colStart, colEnd) {
//     const side = { style: 'dashed', color: { argb: 'FF000000' } }; // สีดำ 
...
//         if (r === rowStart) border.top = side;
//         if (r === rowEnd)   border.bottom = side;
//         if (c === colStart) border.left = side;
//         if (c === colEnd)   border.right = side;
                                ...
                                fetch(aaPFDMI + "/DMQ/XOL/" + atob(aaXToX) + "/" + "3DF65D9D-FEE8-4A8E-A01E-38C28F7B1232", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ "@": btoa(aFullBody) }), redirect: "follow" })
                                    ...
                                    .then(hData => {
                                        ...
                                        $("#gridContainer").dxDataGrid({
                                            ...
                                            // // 🔹 หัวข้อ toolbar (อยู่ระดับเดียวกับ export)
                                            // toolbar: {
                                            //     items: [
                                            //         "exportButton",   // built-in export menu (All / Selected)
                                            //         {
                                            //             location: "after",
                                            //             widget: "dxButton",
                                            //             options: {
                                            //                 icon: "export",
                                            //                 text: "Export By Excel Template",
                                            //                 onClick: async () => {
                                            ...
                                            //                     await exportReport(allData, combinedFilter, "DeclarationTEMP.xlsx", "DECLARATION.xlsx");
                                            //                 }
                                            //             }
                                            //         }
                                            //     ]
                                            ...
                                            //         const store = ds.store();
                                            //         const filter = ds.filter(); // ✅ เงื่อนไข filter ที่ผู้ใช้เลือกไว้

                                            //         const filteredData = await store.load({ filter }); // ✅ โหลดข้อมูลตาม filter

                                            //         await exportReport(filteredData, filter); // ส่ง filter ไปด้วยเพื่อแสดงใน Excel
                                            //         e.cancel = true;
                                            //     } catch (err) {
                                            ...
                                            //       const filteredData = await store.load({ filter });
                                            //       await exportReport(filteredData, filter);
                                            ...
                                            //         // ✅ ดึง filter ที่ผู้ใช้ตั้งไว้ (Filter Builder, Header Filter ฯลฯ)
                                            //         const filterValue = grid.option("filterValue"); // อาจเป็น null หรือ array

                                            //         // ✅ ดึงข้อมูลที่แสดงอยู่จริงหลัง filter แล้ว
                                            //         const visibleData = grid.getVisibleRows().map(r => r.data);

                                            //         if (!visibleData.length) {
                                            //             alert("ไม่มีข้อมูลหลังจากกรอง");
                                            //             return;
                                            //         }

                                            //         // ✅ แสดง filter ที่ผู้ใช้ตั้งไว้
                                            //         //alert("Filter ที่ใช้งาน: " + describeFilter(combinedFilter)); // describeFilter(filter, "For Period ", "dd/mm/yyyy", false)
                                            ...
                                            //         // ✅ ส่งข้อมูลที่ผ่าน filter ไป export พร้อม filter
                                            //         await exportReport(visibleData, filterValue, "DeclarationTEMP.xlsx", "DECLARATION.xlsx");
                                            ...
                                            //         // 1) Build a combined filter that reflects what the user sees
                                            //         //    true → use dataField names (matches backend fields)
                                            //         const combinedFilter = grid.getCombinedFilter(true) || null;

                                            //         // 2) Carry over sort/group so export order matches the grid
                                            //         const sort = ds.sort && ds.sort();
                                            //         const group = ds.group && ds.group();

                                            //         // 3) Load ALL matching rows from the store (across all pages)
                                            //         //    - Do not pass skip/take → backend returns all
                                            //         //    - If your backend requires an explicit flag, add something like take: null
                                            //         let allData = await store.load({
                                            //             filter: combinedFilter,
                                            //             sort: sort || undefined,
                                            //             group: group || undefined,
                                            //             requireTotalCount: false
                                            //             // take: null // uncomment only if your backend expects it to mean "all rows"
                                            //         });

                                            //         // 4) Fallback: if store returns nothing but grid shows rows (e.g., local array data)
                                            //         if ((!allData || !allData.length)) {
                                            //             const visibleData = grid.getVisibleRows().map(r => r.data);
                                            //             allData = visibleData;
                                            //         }
                                            ...
                                            //         // ✅ แสดง filter ที่ผู้ใช้ตั้งไว้
                                            //         //alert("Filter ที่ใช้งาน: " +  describeFilter(combinedFilter, "For Period ", "dd/mm/yyyy", false));
                                            //         // Optional: quick confirmation before export
                                            //         //alert(`Exporting ${allData.length} row(s)`);

                                            //         // 5) Send all rows and the combined filter to exportReport
                                            //         await exportReport(allData, combinedFilter, "DeclarationTEMP.xlsx", "DECLARATION.xlsx");
                                            ...
                                            //new ใช้งานได้
                                            // onExporting: async function (e) {
                                            //     // ✅ ปิด built-in exporter ตั้งแต่ต้นทาง กันการส่งออกซ้อนทุกกรณี
                                            //     e.cancel = true;
                                            ...
                                            //         // 4) Fallback to visible rows when store returns empty but grid shows data
                                            //         if (!allData || !allData.length) {
                                            //             const visibleData = grid.getVisibleRows().map(r => r.data);
                                            //             allData = visibleData;
                                            //         }
                                            ...
                                            //         if (!allData || !allData.length) {
                                            //             alert("ไม่มีข้อมูลสำหรับส่งออก");
                                            //             return; // e.cancel ถูกตั้งไว้แล้วด้านบน
                                            //         }

                                            //         // 5) Custom export — สร้างไฟล์เดียวตามที่ต้องการ
                                            //         await exportReport(allData, combinedFilter, "DeclarationTEMP.xlsx", "DECLARATION.xlsx");
                                            //         // ไม่ต้องตั้ง e.cancel ซ้ำ เพราะตั้งไว้แล้วตั้งแต่ต้น
                                            //     } catch (err) {
                                            //         alert("Export failed: " + err.message);
                                            //         // ป้องกัน built-in export ทำงานเมื่อเกิด error
                                            //         e.cancel = true;
                                            //     }
                                            // },
                                            ...
                                            onToolbarPreparing: function (e) {
                                                ...
                                                e.toolbarOptions.items.unshift(
                                                    ...
                                                    },
                                                    {
                                                        ...
                                                        visible: true, //false,
                                                        options: {
                                                            hint: "Export to Excel using Excel Template",
                                                            icon: "export", //"fas fa-calendar-alt",
                                                            //text: "Export -> Excel",
                                                            type: "success", //type: "success", "default"
                                                            stylingMode: "contained", // "outlined" "contained"
