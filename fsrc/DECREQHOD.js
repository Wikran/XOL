                ...
                .then(result => {
                    ...
                    fetch(aaPFDMI + "/DMQ/XOL/" + atob(aaXToX) + "/" + "3DF65D9D-FEE8-4A8E-A01E-38C28F7B1232", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ "@": btoa(aFullBody) }), redirect: "follow" })
                        ...
                        .then(hData => {
                            ...
                            const aHODApproveSSx = (iData) => { // HOD Approve
                            ...
                            const aHODApproveSS = async (iData) => { // HOD Approve
                                ...
                                // --- Approval logic (unchanged workflow) ---
                                if (anLno === 0) {
                                    aApprovalDateFN = "ERODate02";
                                    if (xxNofChk === 1) {
                                        aaCnfTitle = "APPROVE ?";
                                        aaCnfBody = `Press 'YES' to Approve and Send Mail to ${aRequesterName} Email ${aRequesterEmail}`;
                                        aTrueORFalse = "1"; aTrueORFalseB = true;
                                        aERStatus = aArrays.DECSTATUS[1];
                                    } else {
                                        xxNextApproverxx = xxChkNamexx[anLno + 1];
                                        xxNextAppEmailxx = xxChkEmailxx[anLno + 1];
                                        aaCnfTitle = "VERIFY ?";
                                        aaCnfBody = `Press 'YES' to Verify and Send Mail to ${xxNextApproverxx} [Email ${xxNextAppEmailxx}] <br> For next verify or approve.`;
                                        aTrueORFalse = "0"; aTrueORFalseB = false;
                                        aERStatus = aArrays.DECSTATUS[0];
                                    }
                                } else if (anLno === 1) {
                                    aApprovalDateFN = "ERODate03";
                                    if (xxNofChk === 2) {
                                ...
                                const result = DevExpress.ui.dialog.confirm(aaCnfBody, aaCnfTitle);
                                result.done(async (dresult) => {
                                    ...
                                    let aObjKeyData = {};
                                    if (aApprovalDateFN === "ERODate02") {
                                        aObjKeyData = { REFNO: iData.REFNO, HODApproved: aTrueORFalseB, ERStatus: aERStatus, Vendor02Note: xxNextApproverxx, ERODate02: new Date() };
                                    } else if (aApprovalDateFN === "ERODate03") {
                                        aObjKeyData = { REFNO: iData.REFNO, HODApproved: aTrueORFalseB, ERStatus: aERStatus, Vendor02Note: xxNextApproverxx, ERODate03: new Date() };
                                    } else if (aApprovalDateFN === "ERODate04") {
                                        aObjKeyData = { REFNO: iData.REFNO, HODApproved: aTrueORFalseB, ERStatus: aERStatus, Vendor02Note: xxNextApproverxx, ERODate04: new Date() };
                                    }

                                    const aObjRowData = JSON.stringify({ ...iData, ...aObjKeyData });

                                    // ✅ Await DB update
                                    await sendRequestNew("Update", aObjRowData, aaTBKey, aaPFDMI, atob(aaXToX));

                                    // --- SQL update logic (unchanged workflow) ---
                                    let aSQLCommand = "";
                                    if (aTrueORFalse === "1") {
                                        aSQLCommand = `use ExtraOnLine; UPDATE ${aTableName} SET HODApproved = ${aTrueORFalse}, ERStatus = '${aERStatus}', ${aApprovalDateFN} = '${aNowDateT}' WHERE HeadRefNo = '${iData.HeadRefNo}'`;
                                        await aSQLAction(aaPFDMI, aSQLCommand);

                                        if (iData.ERORefNo1 === "Receives") {
                                            const backupSQL = `USE ExtraOnLine; INSERT INTO ${aOTHTableName} SELECT * FROM ${aTableName} WHERE HeadRefNo = '${aRefNo}';`;
                                            await aSQLAction(aaPFDMI, backupSQL);

                                            const updateSQL = `USE ExtraOnLine; UPDATE ${aOTHTableName} SET HeadRefNo = 'G' + SUBSTRING(HeadRefNo, 2, LEN(HeadRefNo)), REFNO = 'G' + SUBSTRING(REFNO, 2, LEN(REFNO)) WHERE HeadRefNo = '${aRefNo}';`;
                                            await aSQLAction(aaPFDMI, updateSQL);
                                        }
                                    } else {
                                        aSQLCommand = `use ExtraOnLine; UPDATE ${aTableName} SET HODApproved = ${aTrueORFalse}, ERStatus = '${aERStatus}', ${aApprovalDateFN} = '${aNowDateT}', Vendor02Note = '${xxNextApproverxx}', ERORefNo6 = '${xxNextAppEmailxx}' WHERE HeadRefNo = '${iData.HeadRefNo}'`;
                                        await aSQLAction(aaPFDMI, aSQLCommand);
                                    ...
                                    // --- Email sending ---
                                    const aRefNoa = iData.HeadRefNo;
                                    let aMessage01, aaMailTitle, aaMessTitle;
                                    const aApproverName = aaHODAppName;
                                    const aApproverEmail = $.trim(aaHODAppEmail);
                                    const aAddress2Do = `<a href='${aaPFDMI}/XOL/index.html'>Declaration Requisition</a>`;

                                    const getvalues = {
                                        aApproverName,
                                        aaOnInitExpGroupDesc,
                                        aRefNoa,
                                        aAddress2Do,
                                        aRequesterName,
                                        aaMessTitle,
                                    ...
                                    if (aTrueORFalse === "1") {
                                        aaMailTitle = `${aGDescENG.toUpperCase()} - HOD APPROVED`;
                                        aaMessTitle = `${aGDescENG.toUpperCase()} <br> HOD APPROVED`;
                                        aMessage01 = aArrays.ACONFIRM[0].replace(/\${(.*?)}/g, (_, p1) => getvalues[p1] || _);

                                        const aMessage = buildHtmlMessage(aaMessTitle, aMessage01);
                                        await aSendMailDMZ(` ${aRequesterName}`, aRequesterEmail, aApproverEmail, "", "", aaMailTitle, aMessage);
                                    } else {
                                        aaMailTitle = `${aGDescENG.toUpperCase()} - NEED VERIFY/APPROVE`;
                                        aaMessTitle = `${aGDescENG.toUpperCase()} <br> NEED VERIFY/APPROVE`;
                                        aMessage01 = aArrays.ACONFIRM[1].replace(/\${(.*?)}/g, (_, p1) => getvalues[p1] || _);

                                        const aMessage = buildHtmlMessage(aaMessTitle, aMessage01);
                                        await aSendMailDMZ(` ${xxNextApproverxx}`, xxNextAppEmailxx, aApproverEmail, "", "", aaMailTitle, aMessage);
                                    }
                                    ...
                                    // --- Notify user ---
                                    if (aTrueORFalse === "1") {
                                        aMessageAlert(`Already Approved <br> EMAIL <br>${aMessage01}`, "DarkGreen");
                                    } else {
                                        aMessageAlert(`Already Verified and Sent Mail to ${xxNextApproverxx} (${xxNextAppEmailxx})`, "DarkGreen");
                                    }
                                });
                            };
