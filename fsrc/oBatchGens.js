(() => {
    ...
    var aKeyIDa = "MXXXEETRF"; //aaPXIXD; //"main"; //aaPXIXD;
    ...
    LoadSQLData(isLocalHost(), aDatabasea, aKeyIDa, aKeyField, axFieldSelected)
        .then(result => {
            ...
            function createNotePopupContent(noteData, mainRow) { //, popupInstance
                const $content = $("<div style='padding:10px;'></div>");
                const $grid = $("<div id='noteGrid'></div>");
                $content.append($grid);
                                    ...
                                    options: {
                                        ...
                                        onClick: function () {
                                            if (lLoadTrans) {
                                                ...
                                                //aMessageAlert("<p><h2><strong style='color: rgb(125, 2, 2);'>PLEASE</strong><span style='background-color: rgb(250, 247, 227); color: rgb(125, 2, 2);'> </span><strong style='background-color: rgb(217, 250, 243); color: rgb(125, 2, 2);'>LOAD TRANS.</strong><strong style='background-color: rgb(255, 255, 255); color: rgb(125, 2, 2);'> BEFORE</strong><table><tbody><tr><td><p>You have to load excel cash advance transaction before click [PROCESS] </p></td></tr></tbody></table><p style='text-align: left;'></p>", "Red")
                                            }
                                        }
                                    }
                                },
            ...
            const loadUrlTemp = async (Url, filename) => {
                ...
                console.log(serverInfo.hostname)
                console.log(urlObj)
                try {
                    if (serverInfo.hostname === "localhost") {
                        // 👉 ถ้าเป็น localhost ให้ลองโหลดจาก local server
                        const localUrl = `http://localhost:8089/temp/uploads/${filename}`;
                        ...
                        // 👉 โหลดจาก server ปกติ (ต้องแก้ CORS ที่ server)
                    ...
                    // 👉 fallback: ให้ user เลือกไฟล์เอง
                    //$("#fileUploader").dxFileUploader("instance").option("visible", true);
                }
            };

            const loadExcelPopup = () => { // Load Excel Transaction
                // สร้าง popup container ถ้ายังไม่มี
            ...
            // const aULbCustomerGrp = async (customerGroupName) => {
            //     if (!lLoadTemp) return notify("error", "Please load Excel Template before.", 4000);
            //     if (!jsonData?.length) return notify("warning", "No data to export.", 2000);
            //     if (!templateBuffer) return notify("warning", "Load FlexibleTEMP.xlsx first.", 2000);

            //     // Merge summary fields into jsonData
            //     jsonData = combineGroupsToJsonData(jsonData, njsonDataSum);
            ...
            //     // ✅ Only process the requested group
            //     const groupRecords = groups[customerGroupName];
            //     if (!groupRecords) {
            ...
            //         const dataSheet = templateWb.getWorksheet("DATA");
            //         if (!memoSheet || !dataSheet) {
            //             return notify("error", "Template missing MEMO or DATA sheet.", 4000);
            //         }

            //         // 🔑 Everything inside your loop stays the same,
            //         // but replace `groups[groupNameRaw]` with `groupRecords`
            ...
            //         // … keep your memoSheet writing logic unchanged …

            //         // Stamp values into DATA sheet
            //         const firstRecord = groupRecords[0];
            //         if (firstRecord) {
            //             dataSheet.getCell("E5").value = firstRecord.DateText;
            //             dataSheet.getCell("C2").value = customerGroupName;
            //             dataSheet.getCell("C1").value = firstRecord.HeadRefNo;
            //             dataSheet.getCell("C4").value = firstRecord.EROAmount2;
            //             dataSheet.getCell("C7").value = firstRecord.CashAdvance;
            //             dataSheet.getCell("E6").value = firstRecord.ERODesc05;
            //         }
            ...
            //     } catch (err) {
            //         console.error(err);
            //         notify("error", `Failed export for ${customerGroupName}.`, 3000);
            //     }
            // };
            ...
            const ULbCustomerGrp = async (customerGroupName) => {
                ...
                try {
                    ...
                    const safeName = "TemporaryR"//dlRefNO || "Unknown";
                    if (safeName !== "Unknown") {
                        const outBuffer = await templateWb.xlsx.writeBuffer();
                        const outFile = new File(
                            [outBuffer],
            ...
            const uULbCustomerGrp = async (customerGroupName) => {
                if (!lLoadTemp) return notify("error", "Please load Excel Template before.", 4000);
                if (!jsonData?.length) return notify("warning", "No data to export.", 2000);
                if (!templateBuffer) return notify("warning", "Load FlexibleTEMP.xlsx first.", 2000);

                // Preserve master flow: merge summary into jsonData
                console.log(njsonDataSum);
                jsonData = combineGroupsToJsonData(jsonData, njsonDataSum);
                console.log(jsonData);

                njsonDataSum.forEach(entry => {
                    let msg = "ERORefNo1: " + entry.ERORefNo1 + "\n";
                    if (entry.Note && typeof entry.Note === "object") {
                        Object.keys(entry.Note).forEach(key => {
                            const v = entry.Note[key];
                            msg += "ProductName: " + (v.ProductName || key) +
                                " | PayBy: " + (v.PayBy || v.Payby) + "\n";
                        });
                    }
                    alert(msg);
                });
                ...
                // STEP 1: Find njsonDataSum.Note using njsonDataSum.ERORefNo1 linked to group.CompanyGroup
                const noteSource = Array.isArray(njsonDataSum)
                    ? njsonDataSum.find(entry => (entry?.ERORefNo1 || "").trim() === (groupNameRaw || "").trim())
                    : (njsonDataSum && (njsonDataSum.ERORefNo1 || "").trim() === (groupNameRaw || "").trim() ? njsonDataSum : null);

                const noteRaw = noteSource?.Note;

                // STEP 2: Build PayBy lookup by ProductName from the found Note
                const payByDefault = "Corporate Card";
                // const payByMap = (() => {
                //     const map = {};
                //     if (Array.isArray(noteRaw)) {
                //         noteRaw.forEach(item => {
                //             const key = (item?.ProductName || item?.Product || "").trim();
                //             if (key) map[key] = (item?.PayBy ?? item?.Payby ?? payByDefault);
                //         });
                //     } else if (noteRaw && typeof noteRaw === "object") {
                //         Object.keys(noteRaw).forEach(k => {
                //             const v = noteRaw[k];
                //             if (v && typeof v === "object") {
                //                 const key = (v.ProductName || v.Product || k || "").trim();
                //                 if (key) map[key] = (v.PayBy ?? v.Payby ?? payByDefault);
                //             } else {
                //                 const key = (k || "").trim();
                //                 if (key && typeof v === "string") map[key] = v || payByDefault;
                //             }
                //         });
                //     }
                //     return map;
                // })();
                const payByMap = {};
                if (noteSource && Array.isArray(noteSource.Note)) {
                    noteSource.Note.forEach(item => {
                        const key = (item.ProductName || "").trim();
                        if (key) payByMap[key] = item.PayBy || "Corporate Card";
                    });
                }
                ...
                try {
                    ...
                    // Prepare grouped data by company (only selected group)
                    const byCompany = groupRecords.reduce((acc, r) => {
                        const companyName = (r.CompanyName || r.Company || "Unknown").trim();
                        const qty = Number(r.Quantity) || 0;
                        const cost = Number(r.ProductCost) || 0;
                        ...
                        // STEP 3: DateText = PayBy resolved from payByMap by ProductName
                        const pName = (r.ProductName || r.Product || "").trim();
                        const payBy = payByMap[pName] || payByDefault;
                        alert(pName)
                        alert(JSON.stringify(payByMap, null, 2));
                        alert(payBy)
                            (acc[companyName] ||= []).push({
                                ProductName: r.ProductName,
                                Quantity: qty,
                    ...
                    // Summary block (Corporate/Personal/Total) + Regards + Prepared/Approved + Dates + Verify row
                    let summaryStartRow = memoSheet.lastRow.number + 2;
                    const formulaStartRow = 18;
                    const formulaEndRow = summaryStartRow - 1;
                    ...
                    // Merge F:G for Approved by name
                    memoSheet.mergeCells(summaryStartRow - 1, 6, summaryStartRow - 1, 7);
            ...
            function viewUploadedFile(filename) {
                ...
                // Use the popup instance
                viewerPopup.option("title", "Viewing: " + filename);
                viewerPopup.show();
