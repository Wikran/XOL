$(() => {
    ...
    $("#excel_transform").dxButton({
        ...
        onClick: async function () {
            try {
                //remoteUrl = "https://cbsdev2.locktonwattana.com/temp/uploads/FlexibleIME.xlsx";
                const filename = filenameInput.option("value").trim();//"FlexibleIME.xlsx";
                if (!filename) {
                    DevExpress.ui.dialog.alert("Please select a filename.", "error");
                    return;
                }
                ...
                // Download new file Open When Needed
                // const link = document.createElement("a");
                // link.href = URL.createObjectURL(processedFile);
                // link.download = processedFile.name;
                // link.click();
                ...
                // convert to PDF // Example: get PDF Blob from exportSheetToPDF
                // const pdfBlob = exportSheetToPDF(processedWorkbook, "MEMO", pdffilename, false);
                // // Wrap Blob into File
                // const pdfFile = new File([pdfBlob], pdffilename, { type: "application/pdf" });
                // // Upload using your function
                // const pdffile2view = await u2pload2File(pdfFile);
                // if (pdffile2view) {
                //     viewUploadedFile(pdffile2view); // show in PDF viewer
                // }
    ...
    async function ExcelReplace(excelFile, excelUrl, placeHolders, sheetName = "DATA") {
        try {
            //remoteUrl = "https://cbsdev2.locktonwattana.com/temp/uploads/FlexibleIME.xlsx";
