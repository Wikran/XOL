        ...
        $(() => {
            ...
            LoadSQLData(isLocalHost(), aDatabasea, aKeyIDa, aKeyField, axFieldSelected)
                .then(result => {
                    ...
                    console.log(aObjects.aAvatar)
                    //alert(aObjects.aUserStd)
                    var aRightsGroup = aObjects.aRightsGroup;
                    var aRightsAdmin = aObjects.aRightsAdmin;
                    var aAvatar = aObjects.aAvatar;
                    var UserStd = aObjects.aUserStd;
                    var aThemeList = aObjects.aThemeList
                    var aThemeListGroup = new DevExpress.data.DataSource({
                        store: aThemeList,
                        key: "thid",
                        group: "Category"
                    ...
                    let aFullBody = "Select " + aFieldSelected + " From " + "ExtraOnLine.dbo.XOLStaffs " + aqr2S; //aaPFDMZz [lockthbnk-ap14].
                    fetch(aaPFDMI + "/DMQ/" + acPRJ + "/" + atob(aaXToX) + "/" + "3DF65D9D-FEE8-4A8E-A01E-38C28F7B1232", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ "@": btoa(aFullBody) }), redirect: "follow" })
                        ...
                        .then(aData => {
                            ...
                            var aDeveloperEdit = false; //aRolesAction(aRrgSx, nDataPos, 4); // Developer Edit  
                            var ahrSuperEdit = false;
                            var aScF = "/all"
                            var aqrFull = "IDUsr != '' "
                            var nFoundn = aDDeDx.search("admin")
                            var nDevFound = aDDeDx.search("Developer") //aDDeDx.search("faDeveloper") //hrSupervisor faSupervisor
                            if (nDevFound !== -1) { // found
                                aDeveloperEdit = true; // Developer Edit
                            }
                            //alert(nFoundn)
                            ...
                            var nFoundnSfa = aDDeDx.search("faSupervisor") //faSupervisor
                            if (nFoundnShr !== -1 || nFoundnSfa !== -1 ) { //$.trim(aDDeDx) !== "admin"
                                aScF = "/" + $.trim(aaUsrN);
                                var aqrFull = "IDUsr != '' "
                                aUsrEdit = false;
                                aAdminEdit = false;
                            ...
                            $("#gridContainer").dxDataGrid({
                                ...
                                dataSource: new DevExpress.data.CustomStore({
                                    ...
                                    load: function () {
                                        return fetch(aurl, {
                                            method: "POST",
                                            headers: { "Content-Type": "application/json" },
                                            body: JSON.stringify({ "@": btoa(aqrFull) }), //
                                        })
                                            .then(response => {
                                                if (!response.ok) {
                                                    throw new Error('Network response was not ok');
                                ...
                                onEditorPreparing: function (e) {
                                    if (e.parentType === "dataRow" && arDataU === 0) {
                                        ...
                                        if (ahrSuperEdit && e.dataField === "IDUsr" && e.parentType === "dataRow") {
                                            e.editorOptions.disabled = true;
                                        }
                                
                                        // Only add the toggle button if aDeveloperEdit is true
                                        if (e.dataField === "Pword" && e.parentType === "dataRow" ) {
                                            e.editorOptions.mode = "password";
                                
                                            if (aDeveloperEdit === true) {
                                                e.editorOptions.buttons = [{
                                                    ...
                                                    options: {
                                                        ...
                                                        stylingMode: "text",
                                                        onClick: function () {
                                                            const editorInstance = e.component.getCellElement(e.row.rowIndex, e.dataField)
                                                                .find(".dx-texteditor").dxTextBox("instance");
                                
                                                            const currentMode = editorInstance.option("mode");
                                                            const newMode = currentMode === "password" ? "text" : "password";
                                                            const newIcon = newMode === "password" ? " " : " "; //"fas fa-eye" : "fas fa-eye-slash";
                                
                                                            editorInstance.option("mode", newMode);
                                                            editorInstance.option("buttons", [{
                                                                name: "togglePassword",
                                                                location: "after",
                                                                options: {
                                                                    icon: newIcon,
                                ...
                                editing: {
                                    ...
                                    // 👇 This controls the form layout inside the popup
                                    // form: {
                                    //     colCount: 3, // ✅ Change number of columns
                                    //     labelLocation: "top" // optional, for better layout
                                    // },
                                    // 
                                    form: {
                                        colCount: 1, // Top-level layout with 1 column
                                        labelLocation: "top",
                                        items: [
                                            {
                                                itemType: "group",
                                                colCount: 2, // Split layout: left (User Profile), right (Login + Settings)
                                                colSpan: 2,
                                                items: [
                                                    // LEFT COLUMN (User Profile)
                                                    {
                                                        ...
                                                        cssClass: "group-user-info",
                                                        colCount: 2,
                                                        items: [
                                                            { dataField: "Scopebase" }, { dataField: "Tkey" },
                                                            { dataField: "LGName" }, { dataField: "Nickname" },
                                                            { dataField: "Department" }, { dataField: "Division" },
                                                            { dataField: "Description", colSpan: 2 }, // Full width for Description
                                                            { dataField: "email" }, { dataField: "telephone" },
                                                            { itemType: "empty", colSpan: 2,},
                                                        ]
                                                    },

                                                    // RIGHT COLUMN (Login + Settings)
                                                    {
                                                        itemType: "group",
                                                        cssClass: "group-login",
                                                        colCount: 1, // Stack Login + Settings vertically
                                                        items: [
                                                            {
                                                                itemType: "group",
                                                                ...
                                                                colCount: 4,
                                                                items: [
                                                                    { dataField: "IDUsr" }, { dataField: "Pword" }, { dataField: "otp" },
                                                                    { dataField: "Active", colSpan: 1 }
                                                                ]
                                                            },
                                                            {
                                                                itemType: "group",
                                                                caption: "Rights Settings",
                                                                cssClass: "group-settings",
                                                                colCount: 3,
                                                                items: [
                                                                    { dataField: "TGR" },
                                                                    { dataField: "Gright", colSpan: 2 },

                                                                ]
                                                            },
                                                            {
                                                                itemType: "group",
                                                                caption: "Cosmatic Settings",
                                                                cssClass: "group-contact",
                                                                colCount: 3,
                                                                items: [
                                                                    { dataField: "Kright" , colSpan: 1},{ dataField: "PictureLoc", colSpan: 2 }, 
                                                                ]
                                                            },
                                ...
                                columns: [
                                    ...
                                    },
                                    {
                                        ...
                                        editorOptions: { width: 250, readOnly: ahrSuperEdit },
                                        width: 250,
                                        visible: false,
                                    },
                                    {
                                    ...
                                    },
                                    {
                                        ...
                                        groupIndex: 0,
                                        editorOptions: { width: 150, },//readOnly: ahrSuperEdit 
                                        width: 150,
                                    },
                                    {
                                        dataField: "TGR", //"Gright",//
                                        caption: "Rights Selection", //"Rights Select (Select for Group Rights)",
                                        dataType: "string",
                                    ...
                                    },
                                    {
                                        ...
                                        dataType: "string",
                                        editorOptions: { width: 350, showClearButton: true, readOnly: ahrSuperEdit }, //, stylingMode: 'filled'
                                        width: 350,
                                        visible: false,
                                    },
                                    // check if you want to use this field
                                    // {
                                    //     dataField: "Gright",
                                    //     caption: "Group Rights",
                                    //     dataType: "string",
                                    ...
                                    //     setCellValue: function (newData, value, currentRowData) {
                                    //         let aPreGR = "";
                                    //         if (!currentRowData.Gright || currentRowData.Gright === "") {
                                    //             aPreGR = "";
                                    //         } else {
                                    //             aPreGR = currentRowData.Gright + ",";
                                    //         }
                                    ...
                                    },
                                    {
                                        ...
                                        caption: "Password",
                                        dataType: "string",
                                        editorOptions: { width: 150, readOnly: ahrSuperEdit, mode: "password" }, //mode: "password",
                                        width: 150,
                                        allowHiding: aAdminEdit, //false, true = allow
                                        visible: false,
                                    },
                                    {
                                        dataField: "Division",
                                    ...
                                    // },
                                    {
                                        ...
                                        cellTemplate: function (container, options) {
                                            $("<div>").dxCheckBox({ //dxCheckBox dxSwitch
                                                value: options.value,
                                                readOnly: true
                                            }).appendTo(container);
                                        }
                            ...
                            function EditUserProfiles(iData) { //,aaSS,nOfRec
                                ...
                                $(() => { //jQuery(function ($) {
                                    const popup = $("#ChangeProfilePopUp").dxPopup({
                                        ...
                                        resizeEnabled: true,
                                        // onShown: function () {
                                        //     $(".dx-popup").css("border-radius", "16px");
                                        // },
